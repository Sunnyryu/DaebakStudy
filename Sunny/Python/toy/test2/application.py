...
def scheduler():
    ...
    if request.method == 'GET':
        ...
        # testdao.getScheduler에 start와 end를 Dictoionary형식으로 넘겨준다.
        return testdao.getScheduler({'start':start , 'end' : end})
    ...
    if request.method == 'POST':
        ...
        # schedule을 입력한다.
        return  testdao.setScheduler(schedule)

    #요청이 delete면
    if request.method == 'DELETE':
        id = request.form['id']
        return  testdao.delScheduler(id)

    #요청이 put이면
    if request.method == 'PUT':
        schedule = request.form
        return testdao.putScheduler(schedule)
