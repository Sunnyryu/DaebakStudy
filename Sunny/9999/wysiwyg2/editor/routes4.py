...
from editor.db.update import Update_table
...
def product_update():
    ...
    for num in range(len(value_id)):
        if num == 0:
            ...
            rows = Update_table(value_new_id, value_new_title, value_new_body_HTML, value_new_price, value_new_option1_value)#value_new_option2_value)
        else:
            value_new_id = value_id[num].strip(' \r\n\t')
            value_new_price = value_price[num].strip(' \r\n\t')
            value_new_option1_value = value_option1_value[num].strip(' \r\n\t')
    #    value_new_option2_value = value_option2_value[num].strip(' \r\n\t')
            rows = Update_table(value_new_id, value_new_price, value_new_option1_value)
    print(rows)
